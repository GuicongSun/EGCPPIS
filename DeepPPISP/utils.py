...
class ProDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        modified_id = protein_id[:4].lower() + protein_id[4:]

        file_path = f'Feature/dssp/{modified_id}.npy'
        #    检查文件是否存在
        if os.path.exists(file_path):   # 这里主要是重用了Datasets_395的数据集
            dssp = np.load(f'Feature/dssp/{modified_id}.npy')    
            pssm = np.load(f'Feature/pssm/{modified_id}.npy')
            hmm = np.load(f'Feature/hmm/{modified_id}.npy')
            resAF = np.load(f'Feature/resAF/{modified_id}.npy')
            if modified_id == '2j3rA':
                resAF=resAF[:len(hmm),:]    #只有这一个  2j3rA.npy   是多了一维

            bert = np.load(f'Feature/bert_esm2_t36/{modified_id}.npy') #   33        这个最有效，其模型大


            if os.path.exists(f'Feature/distance_map_SC/{modified_id}.npy'):
                distance = np.load(f'Feature/distance_map_SC/{modified_id}.npy')
            else:
                distance = np.load(f'Feature/distance_map/{modified_id}.npy')    #   distance相当于伪位置嵌入，也可以看作节点编码的一种
            ...
            if modified_id!="2j3rA":
                pos = residue_psepos[modified_id]
            else:
                pos = np.zeros((158, 3))


        else:
            dssp = np.load(f'Feature/Feature_422/dssp/{protein_id}.npy')    
            pssm = np.load(f'Feature/Feature_422/pssm/{protein_id}.npy')
            hmm = np.load(f'Feature/Feature_422/hmm/{protein_id}.npy')
            resAF = np.load(f'../Feature/Bio_fea/af_7/{protein_id}.npy')

            bert = np.load(f'../Feature/Bert/bert_esm2_t36/{protein_id}.npy') 


            if os.path.exists(f'../Feature/PDB/pdb_distance_Matrix_422/SC/{protein_id}.npy'):
                distance = np.load(f'../Feature/PDB/pdb_distance_Matrix_422/SC/{protein_id}.npy')
            else:
                distance = np.load(f'../Feature/PDB/pdb_distance_Matrix_422/CA/{protein_id}.npy')   
            distance_map=process_distance_map(distance)
            ...
            pos = np.load(f'../Feature/PDB/psepos/SC/{protein_id}.npy')
        ...
        # 原子特征和原子-残基映射关系
        Atom_path="Tenx/HSSPPI/Atom_model_Train352_Test70/" + protein_id + ".pkl"
